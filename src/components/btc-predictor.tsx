...
export function BTCPredictor() {
  ...
  // Run simulation when historical data is loaded
  useEffect(() => {
    if (historicalData.length > 0 && !isLoadingHistory) {
      runSimulation();
    }
              ...
              <div className="text-left sm:text-right">
                <div className="flex items-center gap-2 mb-1 justify-start sm:justify-end">
                  <p className="text-xs sm:text-sm text-gray-400">
                    {simulationData.aiConfidence ? "AI Predicted Price" : "Predicted Price"}
                  </p>
                  {simulationData.aiConfidence ? (
                    <div className="px-2 py-0.5 bg-purple-500/20 border border-purple-500/30 rounded-full">
                      <span className="text-xs text-purple-300">
                        AI {simulationData.aiConfidence}%
                      </span>
                    </div>
                  ) : (
                ...
                <div className="flex items-center gap-2">
                  <p className="text-xl sm:text-2xl md:text-3xl font-bold text-white">
                    ${simulationData.predictedPrice.toLocaleString()}
                  </p>
                  {priceChange > 0 ? (
      ...
      <Card className="mb-6 bg-gradient-to-r from-purple-900/30 to-pink-900/30 border-purple-500/30 backdrop-blur-sm">
        <CardContent className="py-4">
          <div className="text-center">
            <h2 className="text-2xl font-bold text-white mb-1">1 Week Price Prediction</h2>
            <p className="text-sm text-purple-300">AI-powered 7-day Bitcoin price forecast</p>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          {isSimulating ? (
            <div className="h-64 flex flex-col items-center justify-center gap-4">
              ...
              <p className="text-gray-400 text-sm">Generating simulation...</p>
            </div>
          ) : simulationData ? (
