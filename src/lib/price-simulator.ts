...
import { generateAIPredictions } from './gemini-ai';
...
export async function generateFuturePredictions(
  ...
  if (!predictCount) {
    predictCount = 1; // 1 day for better accuracy
  }
  ...
  if (!aiPrediction || !aiPrediction.predictions) {
    throw new Error('AI prediction failed - please try again later');
  }
...
export async function generatePriceSimulation(
  ...
  const { chartData, aiPrediction } = await generateFuturePredictions(
    historicalData,
    timeframe,
    undefined,
    forceRefresh
  ...
  const baseRecommendation =
    ...
    generateFallbackRecommendation(currentPrice, predictedPrice, priceChange);
...
function generateFallbackRecommendation(
  ...
  if (priceChange < -2) {
    return {
      action: 'SELL',
      entryZone: `$${Math.round(currentPrice - 1000).toLocaleString()} – $${Math.round(
        currentPrice + 1000
      ).toLocaleString()}`,
      target: `$${Math.round(predictedPrice - 3000).toLocaleString()}`,
      stopLoss: `$${Math.round(currentPrice + 3500).toLocaleString()}`,
      reasoning:
        'Statistical analysis suggests bearish momentum. Consider short positions with tight stop-loss.',
    };
  } else if (priceChange > 2) {
    return {
      action: 'BUY',
      entryZone: `$${Math.round(currentPrice - 1500).toLocaleString()} – $${Math.round(
        currentPrice + 500
      ).toLocaleString()}`,
      target: `$${Math.round(predictedPrice + 3000).toLocaleString()}`,
      stopLoss: `$${Math.round(currentPrice - 3500).toLocaleString()}`,
      reasoning:
        'Statistical analysis shows bullish momentum. Entry near current levels recommended.',
    };
  } else {
    return {
      action: 'HOLD',
