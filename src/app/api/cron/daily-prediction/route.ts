...
/**
 * Cron job handler - runs daily at 6:30 AM Myanmar time (00:00 UTC)
 */
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('🤖 Starting daily prediction cron job...');
    ...
    return NextResponse.json({
      success: true,
      message: 'Daily prediction completed',
    ...
    return NextResponse.json(
      {
        success: false,
        error: error.message || 'Failed to run daily prediction',
        timestamp: new Date().toISOString(),
      },
      { status: 500 }
    );
